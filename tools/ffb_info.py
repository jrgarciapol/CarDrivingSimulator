    ...
    if not ent and not raw:
        if not bus:
            ...
            if any(d["pid"] == t300.PID_BOOTLOADER for d in bus):
                ...
                di("    desde Windows con la utilidad de Thrustmaster, pero")
                di("    OJO, no basta con enchufarlo. Hay una secuencia de")
                di("    despertado, y sin ella el actualizador no lo ve:")
                di("      1. instalar el driver y REINICIAR Windows,")
                di("      2. corriente a un enchufe de pared, sin regleta ni")
                di("         alargador, y el USB TODAVIA NO,")
                di("      3. conmutador del volante en PC-PS3,")
                di("      4. mantener pulsados L3 y R3 del volante,")
                di("      5. sin soltarlos, conectar el USB a un puerto")
                di("         DIRECTO del equipo (mejor trasero), esperar tres")
                di("         segundos y soltar,")
                di("      6. abrir 'Firmware Update': debe salir [BOOT].")
                di("    Si no sale nada, repetir entero en otro puerto USB.")
                di("    Tiene que haber un volante montado en la base.")
