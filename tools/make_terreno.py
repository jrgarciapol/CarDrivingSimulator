...
corta (ver simulator/terreno.py) y su reparto de secciones.
    ...
    python tools/make_terreno.py simulator/tracks/m-50.csv [--amplitud=40] [--semilla=1] [--desplazamiento=6]
...
def main(argv):
    ...
    cfg.TRACK_FILE = ruta
    track = Track()
    campo = terreno.generar(track, amplitud=float(ops.get("amplitud", 40.0)),
                            semilla=int(ops.get("semilla", 1)),
                            desplazamiento=float(ops.get("desplazamiento", 6.0)))
